...
class OccupancyGridMap:
    ...
    def is_occupied_idx(self, point_idx):
        ...
        if self.get_data_idx((x_index, y_index)) >= self.occupancy_threshold:
            return True
        elif   0.3<self.get_data_idx((x_index, y_index))<self.occupancy_threshold:
            return False
        else:
            return False
    ...
    #     """
    # Calculate the distance from a given position to the nearest obstacle in the grid map.

    # :param point: A 2D point (x, y) in meters.
    # :return: The distance to the nearest obstacle in grid map units.
    # """
    #     x, y = point
    #     x_index, y_index = self.get_index_from_coordinates(x, y)
    ...
    #     # Calculate the search range based on the maximum diagonal distance
    #     max_search_distance = max(self.dim_meters) * 2**0.5

    #     for i in range(self.dim_cells[0]-2):
    #         for j in range(self.dim_cells[1]-2):
    #             if self.is_occupied_idx((i, j)):
    #                 obstacle_x, obstacle_y = self.get_coordinates_from_index(i, j)
    ...
    #     for i in range(self.dim_cells[0]-2):
    #         if i < 0 or i >= self.dim_cells[0]:
    #             continue

    #         for j in range(self.dim_cells[1]-2):
    #             if j < 0 or j >= self.dim_cells[1]:
    #                 continue
